...
export default function TicketPage() {
  ...
  const movie =
    s.movies != undefined
      ? s.movies.find((movie) => movie.path == screeningInfo.moviePath)
      : null;

  const saloonData = s.saloons.find(
    (saloon) => saloon.name == screeningInfo.auditorium
  );
  const screeningsData = s.screenings.find(
    (screen) => screen.id === screeningInfo.id
  );
  ...
  const getSeats = (numberOfSeatsPerRow, index) => {
    ...
    for (let i = 1; i <= numberOfSeatsPerRow; i++) {
      let check_occupied_seat =
        screeningsData.occupiedSeats[index] !== undefined
          ? screeningsData.occupiedSeats[index].find((element) => element === i)
          : undefined;
      list.push(
        <div
          key={i}
  ...
  return (
    movie && (
      <div className="full-ticket-page">
        {movie != undefined ? (
          <h1 className="detailedTitle">
            Boka platser för filmen: {movie.title}
          </h1>
        ) : null}
        ...
        <div className="seat-selector-container">
          {!saloonData
            ...
            : saloonData.seatsPerRow.map((s, i) => (
                <div className="row">{getSeats(s, i)}</div>
              ))}
          <p className="total-seats">
            Du har valt {clickerss.totalSeats}{" "}
            {clickerss.totalSeats === 1 ? "plats" : "platser"}.
          </p>
