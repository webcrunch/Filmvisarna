...
export default function Booked() {
  ...
  const handleChairsToSave = async () => {
    for (let i = 0; i < bookingInfo.screeningsData.occupiedSeats.length; i++) {
      save.bookedArray.push(
        [
          ...bookingInfo.markedChairsArray[i],
          ...bookingInfo.screeningsData.occupiedSeats[i],
        ].sort()
      );
    }
    let bookingbj = {
      ...
      booking: {
        ...
        date: bookingInfo.screeningsData.date,
        time: bookingInfo.screeningsData.time,
        chairs: placesArray,
      },
    };
      ...
      <div className="confirmation-container">
        <div className="doneA">
          <div className="information">
            ...
            <p>
              Glöm inte att ta med bokningsnummret till biografen:{" "}
              <b>{bookingInfo.confnr}</b>{" "}
              <button
                type="button"
                onClick={() => copyContent(bookingInfo.confnr)}
              >
                Kopiera bokningsnummret
              </button>
            </p>
          </div>
